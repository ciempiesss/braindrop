...
export function Sidebar({ activeTab, onTabChange, dropsForReview, isOpen, onClose }: SidebarProps) {
  return (
    <>
      {isOpen && (
        <div 
      ...
      <aside className={cn(
        "fixed inset-y-0 left-0 z-50 w-[260px] bg-[#0a0a0a] border-r border-[#2f3336] transform transition-transform duration-300 ease-in-out",
        ...
        <div className="p-5 flex items-center justify-between">
          ...
          <button 
            onClick={onClose}
            className="lg:hidden text-2xl text-[#71767b] hover:text-[#e7e9ea]"
          >
            ✕
          </button>
        </div>
